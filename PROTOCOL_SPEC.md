...
1. **The unsolicited "Active Mode" tag-read push frame (`CID1=0x20`) is not documented anywhere in this manual.** No CID1 value `0x20` exists in Table 3-1. The closest match is `RT=0x20` in the Basic Parameters block, which only means "EPC (Gen2) multiple tag" as a *read-type configuration value* — it has no documented relationship to framing an unsolicited push notification. **Status: CID1=0x20 itself is still unverified/undocumented, but the INFO layout is now confirmed** (2026-06-28, against 5 real scans — Bib 21251, 5251, 982, 868 across 3 race/tag providers, plus the manufacturer's own sample/test tag bundled with the reader — all with hand-verified checksums): `INFO = [AN(1B antenna)][PC(2B, standard EPC Gen2 Protocol Control word)][EPC(variable, length = top 5 bits of PC's high byte, in 16-bit words)][RSSI(1B)]`. This replaces an earlier (falsified) assumption that byte 6 was a raw `EPC_LEN` byte — on all 5 real frames, byte 6 was always `0x00` (the AN/antenna byte), and the PC-word-derived length correctly predicted a 12-byte EPC for two of the providers (plus the sample tag) and a 16-byte EPC for the other two, exactly accounting for `LENGTH` with nothing left over every time. The sample tag's decoded EPC (`E2806915000050042B3611EE`) leads with `E2`, the standard Gen2 General Identifier header — independent confirmation the byte boundaries are right, since a wrong offset wouldn't coincidentally land on a real EPC header. The `RTN` byte on these real frames was also consistently `0x05`, not the documented `0x32` ("Auto send to SU") — another data point that this frame type sits outside the manual entirely, even though its INFO layout borrows a real Gen2 standard field (PC word). Implemented in `BleDriver.decodeTagFrame()`; regression-tested against all 5 real frames in `tests/regression.test.js`.
